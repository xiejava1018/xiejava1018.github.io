...
我的个人博客 [xiejava.ishareread.com](http://xiejava.ishareread.com/) 是基于 Hugo 搭建的静态博客，托管在 GitHub Pages 上。虽然 GitHub Pages 免费稳定，但在国内访问速度一直不太理想，DNS 解析慢、连接延迟高，部分地区甚至出现无法访问的情况。
...
- **DNS 解析慢**：GitHub Pages 的域名 `github.io` 使用的是国外的 DNS 服务器，国内解析延迟较高
- **连接延迟高**：服务器部署在海外，国内用户访问需要经过多次跨境跳转
- **部分地区访问不稳定**：受网络环境影响，部分地区可能出现加载缓慢甚至无法访问的情况
- **静态资源加载慢**：CSS、JS、图片等资源都是从海外服务器加载
...
登录[阿里云 ESA 控制台](https://esa.console.aliyun.com/)，点击「添加站点」：

![阿里云 ESA 控制台 - 添加站点](https://image2.ishareread.com/images/2026/20260503/1-esa-add-site.png)
<!-- 截图说明：请截图阿里云 ESA 控制台「添加站点」页面 -->

输入你的域名（如 `ishareread.com`），选择接入方式。ESA 支持两种接入方式：

- **NS 接入**（推荐）：将域名的 NS 记录指向阿里云，由阿里云托管 DNS 解析
- **CNAME 接入**：在原有 DNS 服务商处添加 CNAME 记录指向 ESA
...
![DNS NS 记录修改](https://image2.ishareread.com/images/2026/20260503/2-esa-dns-ns.png)
<!-- 截图说明：请截图域名注册商处修改 NS 记录的页面 -->
...
![ESA DNS 解析配置](https://image2.ishareread.com/images/2026/20260503/3-esa-dns-records.png)
<!-- 截图说明：请截图阿里云 ESA 中 DNS 解析记录列表 -->
![ESA DNS 解析配置](https://image2.ishareread.com/images/2026/20260503/3-esa-dns-records-2.png)
### 4、配置缓存规则
...
![ESA SSL 配置](https://image2.ishareread.com/images/2026/20260503/3-esa-ssl.png)
<!-- 截图说明：请截图 ESA SSL/TLS 配置页面 -->
...
{
    "hugoVersion": "0.160.1",
    "buildCommand": "hugo --minify --buildFuture",
...
登录[腾讯云 EdgeOne Pages 控制台](https://pages.edgeone.ai/)，点击「创建项目」：

![EdgeOne Pages 创建项目](https://image2.ishareread.com/images/2026/20260503/4-edgeone-pages-create.png)
<!-- 截图说明：请截图 EdgeOne Pages 控制台创建项目页面 -->

EdgeOne Pages 提供三种创建方式：

- **通过导入 Git 仓库创建**：适合已有 Hugo 项目（推荐）
- **通过模板创建**：创建全新的 Hugo 站点，使用官方 hugo-starter 模板
...
![EdgeOne Pages 导入仓库](https://image2.ishareread.com/images/2026/20260503/5-edgeone-pages-import.png)
<!-- 截图说明：请截图 EdgeOne Pages 导入 GitHub 仓库选择页面 -->
...
4. 执行构建命令生成静态文件
5. 将构建产物部署到全球边缘节点
![EdgeOne Pages 构建配置](https://image2.ishareread.com/images/2026/20260503/6-edgeone-pages-build.png)
<!-- 截图说明：请截图 EdgeOne Pages 构建配置页面 -->
部署成功后，系统会分配一个 `https://<random>.edgeone.app` 的默认域名。你可以在项目设置中绑定自定义域名：

![EdgeOne Pages 绑定域名](https://image2.ishareread.com/images/2026/20260503/7-edgeone-pages-domain.png)
<!-- 截图说明：请截图 EdgeOne Pages 绑定自定义域名页面 -->
...
![EdgeOne Pages 部署日志](https://image2.ishareread.com/images/2026/20260503/7-edgeone-pages-deploy.png)
<!-- 截图说明：请截图 EdgeOne Pages 部署成功后的构建日志页面 -->
...
- **首页加载**从 0.762s 降到了 0.117s，提升了近 **85%**
- **分类页**从 0.745s 降到了 0.181s，提升了 **76%**
...
- [WebPageTest](https://www.webpagetest.org/) — 最详细，支持选择全球不同节点，提供瀑布图分析
- [GTmetrix](https://gtmetrix.com/) — 基于 Lighthouse，给出性能评分和优化建议
- [PageSpeed Insights](https://pagespeed.web.dev/) — Google 官方测速工具，核心 Web 指标评分

**国内多节点测速：**

- [卡卡网](https://www.webkaka.com/webCheck.aspx) — 国内老牌多节点测速
- [Web测Ping](https://www.webceping.com/) — 全国节点首次访问和缓存访问测速

**DNS / CDN 检测：**

- [What's My DNS](https://www.whatsmydns.net/) — 全球 DNS 解析对比
- [IPIP.net](https://www.ipip.net/ping.php) — 全国 Ping 延迟测试
...
- **页面秒开**：原本需要 0.5~0.8s 才能加载的页面，现在基本在 0.1~0.2s 内完成
- **访问稳定**：不再出现偶尔打不开的情况，全国各地访问速度一致
- **搜索友好**：更快的加载速度有助于搜索引擎收录和排名
...
![“fullbug”微信公众号](https://image2.ishareread.com/images/fullbug微信公众号.jpg "“fullbug”微信公众号") 
